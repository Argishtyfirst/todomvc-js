(function closure() {
  ...
  this.completed ? 'checked' : ''
    ...
    `;
      ...
      qs('input.toggle', li).addEventListener(
        'click',
        this.handleCompleteToggle.bind(this),
      );
      qs('button.destroy', li).addEventListener('click', () =>
        this.onDelete(this));
      qs('.view label', li).addEventListener('dblclick', () =>
        this.onEdit(this));
    ...
    makeAllComplete() {
      ...
      if (this.completedTasks === this.totalTasks && this.completedTasks !== 0) {
    ...
    handleCompleteToggle(event) {
      ...
      if (this.leftTasks === 1) {
        ...
        qs('.todo-count').innerHTML = `<span class="todo-count"><strong>${
          this.leftTasks
  ...
  class TodoMVC {
    ...
    dump() {
      const serializedTasks = this.tasks.map(({ id, title, completed }) => ({
        id,
        title,
        completed,
      }));
    ...
    restore() {
      try {
        ...
        serializedTasks.forEach((task) => {
          this.addNewTask(task.title, {
            completed: task.completed,
            id: task.id,
          });
        });

        const maxId = serializedTasks
          .map(({ id }) => id)
          .sort((a, b) => b - a)[0];
