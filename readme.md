...
* `npm start` to start the server with live reload
* `npm run lint` to check linting errors
* `npm run lint:fix` to fix linting errors if possible
...
The routing should be based on `location.hash`. The states are:
...
Use can listen on hash change events using [hashchange](https://developer.mozilla.org/en-US/docs/Web/Events/hashchange) event on `window`.
